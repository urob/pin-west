...
Pin revisions in [west manifests](https://docs.zephyrproject.org/latest/develop/west/manifest.html)
to exact commit SHAs, check their health, and update them — think
[pinact](https://github.com/suzuki-shunsuke/pinact), but for west. It needs neither an initialized
workspace nor any clones (refs resolve via `git ls-remote`), and it only rewrites `revision:`
values, keeping all other formatting and comments byte-for-byte.
...
Commands that talk to the GitHub API (`bump` and `check`) find a token via `--gh-token`, then
`$GITHUB_TOKEN` / `$GH_TOKEN`, then `gh auth token`; without one, unauthenticated requests are
rate-limited to 60/hour. Non-GitHub remotes work throughout — they just skip the release lookup and
the ancestry check.

## Tracking

Pinned revisions get a trailing comment recording what they track —
`# v0.3.0` for tags, `# main (2026-08-17)` for branches (with the pin date).
`bump` classifies each pin and follows its declared intent (verified against the
remote):
...
For unpinned projects, the declared ref is taken from the manifest's `revision`
field. Names that are both a branch and a tag resolve to the tag. Pinned
projects without a comment are interpreted as releases if a matching release tag
exists, and otherwise fall back to latest-release → highest version tag →
default branch. 

`check --comments` audits the tracking comments: exact-release pins must match
the tag's commit, floating-release pins must pin a member of the release series,
and branch pins must be ancestors of the branch's head (GitHub only).

## Imported projects

By default, only projects explicitly specified in the manifest are pinned or
updated. Use `pin --include-imports` to resolve and lock the manifest's full
import-tree.
...
tree of that project. Deleting the dependency section turns off dependency 
locking.
